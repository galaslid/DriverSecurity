...
public class DriverMonitor : IDisposable
{
    ...
    private async Task CheckDriver(string driverPath)
    {
        try
        {
            ...
            _logger.LogInformation("Driver {DriverPath} vulnerability status: {Status}", 
                driverPath, report.LolDriversStatus);
            ProcessDriverReport(report);
        }
        catch (Exception ex)
        {
    ...
    private void ProcessDriverReport(DriverSecurityReport report)
    {
        if (report.OverallStatus == SecurityStatus.Critical || report.OverallStatus == SecurityStatus.Danger)
        {
            _logger.LogWarning("Critical driver found: {DriverName} at {DriverPath}", report.DriverName, report.DriverPath);
            _logger.LogWarning("Security Status: {Status}", report.OverallStatus);
            _logger.LogWarning("Signature Status: {Status}", report.SignatureStatus);
            _logger.LogWarning("LOLDrivers Status: {Status}", report.LolDriversStatus);
            _logger.LogWarning("Recommendations: {Recommendations}", report.Recommendations);
        }
    }
